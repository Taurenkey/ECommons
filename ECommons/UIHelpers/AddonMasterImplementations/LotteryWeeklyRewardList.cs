...
public unsafe partial class AddonMaster
{
    public class LotteryWeeklyRewardList : AddonMasterBase<AtkUnitBase>
    {
        ...
        public string Week => MemoryHelper.ReadSeStringNullTerminated((nint)Addon->AtkValues[1].String).ExtractTextEC();
        ...
        public readonly struct Reward(LotteryWeeklyRewardList am, int index)
        {
            ...
            public string Place { get; init; } = MemoryHelper.ReadSeStringNullTerminated((nint)am.Addon->AtkValues[11 + 7 * index].String).ExtractTextEC();
            ...
            public string? ItemRewardName { get; init; } = am.Addon->AtkValues[15 + 7 * index].Type == 0 ? null : MemoryHelper.ReadSeStringNullTerminated((nint)am.Addon->AtkValues[15 + 7 * index].String).ExtractTextEC();
            public string Requirement { get; init; } = MemoryHelper.ReadSeStringNullTerminated((nint)am.Addon->AtkValues[16 + 7 * index].String).ExtractTextEC();
        }
